...
object DateTimeFormatter {
  ...
  def formatDateTimeDowntimeHistory2(dateTime: LocalDateTime)(implicit messages: Messages): String =
    s"${dateTime.format(DateTimeGen.ofPattern("h:mma")).toLowerCase(Locale.ENGLISH)}"

  def formatDateTimeDowntimeHistory1(dateTime: LocalDateTime)(implicit messages: Messages): String =
